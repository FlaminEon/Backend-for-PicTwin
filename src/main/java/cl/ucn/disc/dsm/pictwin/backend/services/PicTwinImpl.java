...
public class PicTwinImpl implements PicTwin {
     ...
     */
    @Autowired
    public PicTwinImpl(PicRepository picRepository,
    ...
    public User authenticate(@NonNull String email, @NonNull String password) {
        ...
        return userOptional.orElseThrow(()
                -> new RuntimeException("Wrong Credentials or User Not Found"));
    }
    ...
    public Twin createTwin(@NonNull Pic pic, @NonNull Long idUser) {
        ...
        // Remove my own pics.
        List<Pic> picsFiltered = pics.stream().filter(p ->
                !p.getOwner().getId().equals(idUser)).toList();
        ...
        // Select a random pic.
        Pic your = picsFiltered.size() == 0 ? pic
                : picsFiltered.get(RANDOM.nextInt(picsFiltered.size()));
    ...
    public void dislike(@NonNull Long idTwin, @NonNull Long idUser) {
        ...
        // Check if it exists.
        Twin twin = optionalTwin.orElseThrow(()
                -> new RuntimeException("Cannot find twin with this id: " + idTwin));

        // Check the owner of the pic.
        if (!idUser.equals(twin.getMy().getOwner().getId())) {
            throw new RuntimeException("Twin id<" + idTwin + "> not owned by User id<"
                    + idUser + ">!");
        }
    ...
    /**
     * Used to return the number of users in the repository.
     *
